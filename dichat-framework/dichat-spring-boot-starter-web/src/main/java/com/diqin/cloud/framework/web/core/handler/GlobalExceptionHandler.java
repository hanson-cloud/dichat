package com.diqin.cloud.framework.web.core.handler;
...
import com.diqin.cloud.framework.web.core.util.WebFrameworkUtils;
...
import static com.diqin.cloud.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    public CommonResult<?> allExceptionHandler(HttpServletRequest request, Throwable ex) {
        ...
        if (ex instanceof MaxUploadSizeExceededException) {
            return maxUploadSizeExceededExceptionHandler();
        }
    ...
    public CommonResult<?> methodArgumentTypeInvalidFormatExceptionHandler(HttpMessageNotReadableException ex) {
        ...
        if (StrUtil.startWith(ex.getMessage(), "Required request body is missing")) {
            return CommonResult.error(BAD_REQUEST.code(), "请求参数类型错误: request body 缺失");
        }
    ...
    public CommonResult<?> maxUploadSizeExceededExceptionHandler() {
        return CommonResult.error(BAD_REQUEST.code(), "上传文件过大，请调整后重试");
    }
    ...
    public CommonResult<?> httpMediaTypeNotSupportedExceptionHandler(HttpMediaTypeNotSupportedException ex) {
        ...
        return CommonResult.error(BAD_REQUEST.code(), String.format("请求类型不正确:%s", ex.getMessage()));
    }
    ...
    public CommonResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
        ...
        // 例如说：https://gitee.com/zhijiantianya/dichat-cloud/issues/ICSSRM、https://gitee.com/zhijiantianya/dichat-cloud/issues/ICT6FM
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("mp_")) {
            log.error("[微信公众号 dichat-module-mp - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.code(),
                    "[微信公众号 dichat-module-mp - 表结构未导入]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统 dichat-module-mall - 已禁用]");
            return CommonResult.error(NOT_IMPLEMENTED.code(),
                    "[商城系统 dichat-module-mall - 已禁用]");
        }
        // 9. IM 即时通讯
        if (message.contains("im_")) {
            log.error("[IM 即时通讯 dichat-module-im - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.code(),
                    "[IM 即时通讯 dichat-module-im - 表结构未导入]");
        }
        // 10. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块 dichat-module-pay - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.code(),
                    "[支付模块 dichat-module-pay - 表结构未导入]");
        }
        // 11. AI 大模型
        if (message.contains("ai_")) {
            log.error("[AI 大模型 dichat-module-ai - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.code(),
                    "[AI 大模型 dichat-module-ai - 表结构未导入]");
        }
        return null;
    }
