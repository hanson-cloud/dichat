...
public class ImPrivateMessageServiceImplTest extends BaseMockitoUnitTest {
    ...
    private AppImPrivateMessageSendReqVO buildSendReqVO() {
        AppImPrivateMessageSendReqVO reqVO = new AppImPrivateMessageSendReqVO();
    ...
    public void testSendMessage_success() {
        // 准备
        AppImPrivateMessageSendReqVO reqVO = buildSendReqVO();
        when(privateMessageMapper.selectBySenderIdAndClientMessageId(ImMessageParticipantTypeEnum.USER.getType(), 1L, "test-uuid-001"))
                .thenReturn(null);
    ...
    public void testSendMessage_notFriend() {
        ...
        assertEquals(FRIEND_NOT_FRIEND.code(), exception.getCode());
    }
    ...
    public void testPullMessages_sizeExceeded() {
        ...
        assertEquals(MESSAGE_PULL_SIZE_EXCEEDED.code(), exception.getCode());
    }
    ...
    public void testReadMessages_success() {
        ...
        // 第一次：发给自己的 READ 事件
        assertEquals(1L, userCaptor.getAllValues().getFirst());
        ImPrivateMessageDTO readPayload = contentCaptor.getAllValues().getFirst();
    ...
    public void testRecallMessage_notOwn() {
        ...
        assertEquals(MESSAGE_RECALL_DENIED.code(), exception.getCode());
    }
    ...
    public void testRecallMessage_alreadyRecalled() {
        ...
        ServiceException exception = assertThrows(ServiceException.class,
                () -> privateMessageService.recallPrivateMessage(1L, 10L));
        assertEquals(MESSAGE_ALREADY_RECALLED.code(), exception.getCode());
    }
    ...
    public void testRecallMessage_notExists() {
        ...
        ServiceException exception = assertThrows(ServiceException.class,
                () -> privateMessageService.recallPrivateMessage(1L, 10L));
        assertEquals(MESSAGE_NOT_EXISTS.code(), exception.getCode());
    }
    ...
    public void testRecallMessage_timeout() {
        ...
        ServiceException exception = assertThrows(ServiceException.class,
                () -> privateMessageService.recallPrivateMessage(1L, 10L));
        assertEquals(MESSAGE_RECALL_TIMEOUT.code(), exception.getCode());
    ...
    public void testSendMessage_sensitiveWordBlocked() {
        ...
        ServiceException exception = assertThrows(ServiceException.class,
                () -> privateMessageService.sendPrivateMessage(1L, reqVO));
        assertEquals(MESSAGE_SENSITIVE_WORD_BLOCKED.code(), exception.getCode());
        // 断言：不入库、不推送
        verify(privateMessageMapper, never()).insert(any(ImPrivateMessageDO.class));
    ...
    public void testReadMessages_disabled() {
        ...
        ServiceException exception = assertThrows(ServiceException.class,
                () -> privateMessageService.readPrivateMessages(1L, 2L, 5L));
        assertEquals(MESSAGE_PRIVATE_READ_DISABLED.code(), exception.getCode());
        // 断言：不更新消息状态、不推送
    ...
    public void testGetMaxReadMessageId_hit() {
        // 准备：对方读到我发的最大 id=10
        when(friendService.getLastReadMessageId(1L, 2L)).thenReturn(10L);
    ...
    public void testGetMaxReadMessageId_miss() {
        // 准备：对方一条都没读过
        when(friendService.getLastReadMessageId(1L, 2L)).thenReturn(null);
    ...
    public void testGetPrivateMessageList_delegatesToMapper() {
        // 准备
        AppImPrivateMessageListReqVO reqVO = new AppImPrivateMessageListReqVO();
        reqVO.setReceiverId(2L);
        reqVO.setMaxId(100L);
        reqVO.setLimit(20);
        ...
        when(privateMessageMapper.selectHistoryListByPeerIds(eq(1L), eq(List.of(2L)), eq(100L), eq(20))).thenReturn(mockList);
        ...
        // 断言：透传到 mapper，参数一致
        assertEquals(2, result.size());
        verify(privateMessageMapper).selectHistoryListByPeerIds(eq(1L), eq(List.of(2L)), eq(100L), eq(20));
    }
