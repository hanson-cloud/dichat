...
import static com.diqin.cloud.module.im.dal.redis.RedisKeyConstants.GROUP;
...
public class ImGroupServiceImpl implements ImGroupService {
    ...
    public ImGroupDO createGroup(AppImGroupCreateReqVO createReqVO, Long userId) {
    ...
    @Transactional(rollbackFor = Exception.class)
    public ImGroupDO updateGroup(AppImGroupUpdateReqVO updateReqVO, Long userId) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void inviteGroupMember(Long userId, AppImGroupMemberInviteReqVO inviteReqVO) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void removeGroupMember(Long userId, AppImGroupMemberRemoveReqVO removeReqVO) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void addGroupAdmin(Long userId, AppImGroupAdminAddReqVO reqVO) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void removeGroupAdmin(Long userId, AppImGroupAdminRemoveReqVO reqVO) {
        Long groupId = reqVO.getId();
        Set<Long> targetUserIds = new HashSet<>(reqVO.getUserIds());
        // 1.1 仅群主可操作
        validateGroupOwnerForUpdate(groupId, userId);
    ...
    @Transactional(rollbackFor = Exception.class)
    public void transferGroupOwner(Long userId, AppImGroupTransferOwnerReqVO transferReqVO) {
    ...
    private void validateGroupOwnerForUpdate(Long groupId, Long userId) {
        ImGroupDO group = validateGroupExistsForUpdate(groupId);
        if (ObjUtil.notEqual(group.getOwnerUserId(), userId)) {
            throw exception(GROUP_NOT_OWNER);
        }
    ...
    @Override
    public List<ImGroupDO> getMyGroupList(Long userId, String keyword) {
        ...
        // 2. 批量查询群信息，过滤掉已解散的群（status=DISABLE）
        //    主动退群但群仍活跃的场景不受影响（群 status 仍为 ENABLE）
        //    已解散群的群名/头像等历史信息，前端可通过 /group/get/{id} 按需获取
        Set<Long> groupIds = convertSet(members, ImGroupMemberDO::getGroupId);
        List<ImGroupDO> groups = filterList(groupMapper.selectByIds(groupIds),
                group -> CommonStatusEnum.ENABLE.getStatus().equals(group.getStatus()));
        // 3. 关键字过滤（Java 侧 LIKE，避免对 groupIds 子查询改写）；keyword 为空或 trim 后为空则放行
        String k = keyword == null ? null : keyword.trim();
        if (StrUtil.isBlank(k)) {
            return groups;
        }
        String lower = k.toLowerCase();
        return filterList(groups, g ->
                (g.getName() != null && g.getName().toLowerCase().contains(lower))
                || (g.getNotice() != null && g.getNotice().toLowerCase().contains(lower)));
    }

    @Override
    public PageResult<ImGroupDO> searchPublicGroups(String keyword, String category, Integer pageNo, Integer pageSize) {
        // 兜底分页参数：pageNo>=1，pageSize 1~100
        int pNo = (pageNo == null || pageNo < 1) ? 1 : pageNo;
        int pSize = (pageSize == null || pageSize < 1) ? 20 : Math.min(pageSize, 100);
        PageParam pageParam = new PageParam(pNo, pSize);
        return groupMapper.selectPublicGroups(keyword, category, pageParam);
    }
    ...
    @Transactional(rollbackFor = Exception.class)
    public void muteAll(Long userId, AppImGroupMuteAllReqVO reqVO) {
        // 1. 校验群主或管理员
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void muteMember(Long userId, AppImGroupMuteMemberReqVO reqVO) {
        // 1.1 不能禁言自己
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void cancelMuteMember(Long userId, AppImGroupCancelMuteMemberReqVO reqVO) {
        // 1.1 校验群存在且未封禁
        validateGroupExists(reqVO.getId());
        // 1.2 校验操作人和目标都在群中
        ImGroupMemberDO operatorMember = groupMemberService.validateMemberInGroup(reqVO.getId(), userId);
