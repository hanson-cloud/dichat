...
import com.diqin.cloud.module.im.dal.mysql.rtc.ImRtcParticipantMapper;
import com.diqin.cloud.module.im.dal.redis.rtc.ImRtcCallLockRedisDAO;
...
import com.diqin.cloud.module.im.enums.rtc.ImRtcCallEndReasonEnum;
import com.diqin.cloud.module.im.enums.rtc.ImRtcCallStatusEnum;
import com.diqin.cloud.module.im.enums.rtc.ImRtcParticipantRoleEnum;
...
import com.diqin.cloud.module.im.framework.rtc.core.LiveKitWebhookEventDTO;
...
import com.diqin.cloud.module.im.service.websocket.dto.notification.rtc.*;
...
public class ImRtcCallServiceImpl implements ImRtcCallService {
    ...
    public ImRtcCallDO createCall(Long userId, AppImRtcCallCreateReqVO reqVO) {
        validateEnabled();
        // 1. 校验入参与场景
     ...
     */
    private ImRtcCallDO createGroupCall(Long userId, AppImRtcCallCreateReqVO reqVO) {
     ...
     */
    private ImRtcCallDO createPrivateCall(Long userId, AppImRtcCallCreateReqVO reqVO, Long peerUserId) {
    ...
    @SneakyThrows
    @Transactional(rollbackFor = Exception.class)
    public void inviteCall(Long userId, AppImRtcCallInviteReqVO reqVO) {
        validateEnabled();
        // 1.1 校验通话存在且活跃
     ...
     * @return 通话主表
     */
    private ImRtcCallDO createCall0(Long inviterId, AppImRtcCallCreateReqVO reqVO) {
        ...
        ImUserDO inviterUser = userService.getUser(inviterId);
        Map<Long, ImUserDO> inviteeMap = userService.getUserMap(invitees);
        for (Long inviteeId : invitees) {
    ...
    private void addInvitees(ImRtcCallDO call, Long inviterId, Collection<Long> inviteeIds) {
        ...
        ImUserDO inviter = userService.getUser(inviterId);

        Map<Long, ImUserDO> inviteeMap = userService.getUserMap(incomingUserIds);
    ...
    public String signCallToken(Long userId, String room) {
        ...
        return signToken(userId, resolveDisplayName(userService.getUser(userId), userId), room);
    }
     ...
     * @param userMap     候选用户预查 map；避免逐个 userService.getUser 走 N+1
     ...
     * @param reqVO  创建请求
     */
    private void validateCreateCall(Long userId, AppImRtcCallCreateReqVO reqVO) {
     ...
     */
    private Set<Long> resolveInvitees(AppImRtcCallCreateReqVO reqVO, Long inviterId) {
     ...
     */
    private void pushCallInviteNotification(ImRtcCallDO call, ImUserDO inviter,
                                            Long inviteeId, ImUserDO invitee,
    ...
    private void pushCallRejectNotification(ImRtcCallDO call, Long operatorUserId) {
        ImUserDO operator = operatorUserId != null ? userService.getUser(operatorUserId) : null;
     ...
     */
    private void pushCallNoAnswerNotification(ImRtcCallDO call, Long operatorUserId, ImUserDO operator) {
     ...
     */
    private void pushCallStartNotification(ImRtcCallDO call, ImUserDO inviter, Set<Long> invitees) {
    ...
    private void pushCallEndNotification(ImRtcCallDO call, Long operatorId, ImRtcCallEndReasonEnum reason,
        ...
        ImUserDO operator = operatorId != null ? userService.getUser(operatorId) : null;
     ...
     * @return 房内显示名
     */
    private static String resolveDisplayName(ImUserDO user, Long userId) {
