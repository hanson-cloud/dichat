...
import com.diqin.cloud.module.im.enums.group.ImGroupRequestHandleResultEnum;
...
import com.diqin.cloud.module.im.service.websocket.dto.notification.group.GroupRequestApprovedNotification;
import com.diqin.cloud.module.im.service.websocket.dto.notification.group.GroupRequestReceivedNotification;
import com.diqin.cloud.module.im.service.websocket.dto.notification.group.GroupRequestRejectedNotification;
...
public class ImGroupRequestServiceImpl implements ImGroupRequestService {
    ...
    public ImGroupRequestDO applyJoinGroup(Long userId, AppImGroupRequestApplyReqVO reqVO) {
        ...
        ImUserDO applyUser = userService.getUser(userId);
    ...
    public void createInviteRequestList(Long groupId, Long inviterUserId, Collection<Long> invitedUserIds) {
        ...
        Map<Long, ImUserDO> userMap = userService.getUserMap(invitedUserIds);
        List<Long> ownerAndAdmins = getGroupMemberListByOwnerAndAdminUserIds(group);
        for (ImGroupRequestDO request : requests) {
            ImUserDO applyUser = userMap.get(request.getUserId());
            GroupRequestReceivedNotification payload = buildRequestNotification(group, request, applyUser);
     ...
     * @return 申请记录
     */
    private ImGroupRequestDO createOrResetApplyRequest(Long groupId, Long userId, AppImGroupRequestApplyReqVO reqVO) {
     ...
     * @param reqVO   申请请求
     */
    private void resetApplyRequest(ImGroupRequestDO request, AppImGroupRequestApplyReqVO reqVO) {
        // 1. 更新申请内容、来源和处理状态
        LocalDateTime now = LocalDateTime.now();
