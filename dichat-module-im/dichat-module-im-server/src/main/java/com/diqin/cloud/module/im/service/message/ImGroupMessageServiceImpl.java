...
import com.diqin.cloud.module.im.dal.redis.message.ImGroupMessageReadRedisDAO;
import com.diqin.cloud.module.im.enums.group.ImGroupMemberRoleEnum;
import com.diqin.cloud.module.im.enums.message.ImGroupMessageReceiptStatusEnum;
...
import com.diqin.cloud.module.im.service.websocket.dto.notification.group.GroupChatClearNotification;
import com.diqin.cloud.module.im.service.websocket.dto.notification.group.GroupMessageDeleteNotification;
...
public class ImGroupMessageServiceImpl implements ImGroupMessageService {
    ...
    @Override
    public ImGroupMessageDO sendGroupMessage(Long senderId, AppImGroupMessageSendReqVO reqVO) {
        ...
        if (ImMessageTypeEnum.isMediaMessage(reqVO.getType())) {
            messageReviewManagerService.autoCreateReview(message.getId(), 2,
        ...
        // 多媒体消息走先审后发 → 仅推送给发送方（其余成员等到审核通过后由 reviewService 推）
        if (ImMessageTypeEnum.isMediaMessage(reqVO.getType())) {
            imWebSocketService.sendGroupMessageAsync(Set.of(senderId), ImGroupMessageDTO.ofSend(message));
        } else {
    ...
    @Override
    public List<ImGroupMessageDO> getGroupMessageList(Long userId, AppImGroupMessageListReqVO reqVO) {
        // 1. 校验用户在群中
     ...
     */
    private String normalizeQuoteContent(AppImGroupMessageSendReqVO reqVO, ImGroupMemberDO senderMember) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int deleteGroupMessages(Long userId, Long groupId, List<Long> messageIds) {
        ...
        // 2. 加载待删消息并校验归属
        List<ImGroupMessageDO> messages = groupMessageMapper.selectByIds(messageIds);
        if (CollUtil.isEmpty(messages)) {
            return 0;
        }
        // 2.1 校验：所有 messageId 都归属于该群
        Set<Long> requestedIds = CollUtil.newHashSet(messageIds);
        // 按发送人分桶：自己发的直接删；他人发的需 owner/admin 权限
        List<Long> selfMessageIds = new ArrayList<>();
        List<Long> otherMessageIds = new ArrayList<>();
        for (ImGroupMessageDO message : messages) {
            if (ObjUtil.notEqual(message.getGroupId(), groupId)) {
                throw exception(MESSAGE_NOT_IN_GROUP);
            }
            requestedIds.remove(message.getId());
            if (ObjUtil.equal(message.getSenderId(), userId)) {
                selfMessageIds.add(message.getId());
            } else {
                otherMessageIds.add(message.getId());
            }
        }
        // 2.2 校验：所有请求 messageId 都被加载到了
        if (CollUtil.isNotEmpty(requestedIds)) {
            throw exception(MESSAGE_NOT_EXISTS);
        }
        // 2.3 非 owner/admin 越权删他人消息
        if (!isOwnerOrAdmin && CollUtil.isNotEmpty(otherMessageIds)) {
            throw exception(MESSAGE_DELETE_DENIED);
        }

        // 3. 物理删除：自己消息走 mapper；他人消息（owner/admin 场景）按 groupId+id 单条删
        int deleted = 0;
        if (CollUtil.isNotEmpty(selfMessageIds)) {
            deleted += groupMessageMapper.deleteByGroupIdAndSenderIdAndIds(groupId, userId, selfMessageIds);
        }
        if (CollUtil.isNotEmpty(otherMessageIds) && isOwnerOrAdmin) {
            for (Long messageId : otherMessageIds) {
                deleted += groupMessageMapper.delete(new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<ImGroupMessageDO>()
                        .eq("id", messageId).eq("group_id", groupId));
            }
        }
        log.info("[deleteGroupMessages][userId({}) groupId({}) selfIds={} otherIds={} deleted={} isOwnerOrAdmin={}]",
                userId, groupId, selfMessageIds, otherMessageIds, deleted, isOwnerOrAdmin);

        // 4. WebSocket 多端同步：仅推 userId 自己多端
        List<Long> allDeletedIds = new ArrayList<>(selfMessageIds.size() + otherMessageIds.size());
        allDeletedIds.addAll(selfMessageIds);
        allDeletedIds.addAll(otherMessageIds);
        imWebSocketService.sendGroupMessageAsync(userId,
                buildGroupNotificationDTO(userId, groupId, allDeletedIds, ImMessageTypeEnum.GROUP_MESSAGE_DELETE));
        return deleted;
    }

    /**
     * 构造群消息删除 / 群会话清空 DTO（不入消息流，仅推多端）
     */
    private ImGroupMessageDTO buildGroupNotificationDTO(Long userId, Long groupId, List<Long> messageIds,
                                                        ImMessageTypeEnum type) {
        Object payload;
        if (ImMessageTypeEnum.GROUP_MESSAGE_DELETE.equals(type)) {
            GroupMessageDeleteNotification n = new GroupMessageDeleteNotification();
            n.setOperatorUserId(userId);
            n.setGroupId(groupId);
            n.setMessageIds(messageIds);
            payload = n;
        } else {
            GroupChatClearNotification n = new GroupChatClearNotification();
            n.setOperatorUserId(userId);
            n.setGroupId(groupId);
            payload = n;
        }
        return new ImGroupMessageDTO()
                .setType(type.getType()).setSenderId(userId).setGroupId(groupId)
                .setContent(com.diqin.cloud.framework.common.util.json.JsonUtils.toJsonString(payload))
                .setSendTime(java.time.LocalDateTime.now());
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public int clearGroupChat(Long userId, Long groupId) {
        // 1. 校验群存在 + 用户仍在群中
        groupService.validateGroupExists(groupId);
        groupMemberService.validateMemberInGroup(groupId, userId);

        // 2. 物理删除 groupId 群中所有消息（DB 物理删除后，其他成员本地缓存下次拉取时自然缺失）
        int deleted = groupMessageMapper.deleteByGroupId(groupId);
        log.info("[clearGroupChat][userId({}) groupId({}) deleted={}]", userId, groupId, deleted);

        // 3. WebSocket 多端同步：仅推 userId 自己多端
        imWebSocketService.sendGroupMessageAsync(userId,
                buildGroupNotificationDTO(userId, groupId, null, ImMessageTypeEnum.GROUP_CHAT_CLEAR));
        return deleted;
    }
    ...
    @Override
    public List<ImGroupMessageDO> getGroupMessageExportList(ImGroupMessageManagerPageReqVO reqVO) {
        return groupMessageMapper.selectExportList(reqVO);
    }
