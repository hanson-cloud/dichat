...
public interface ImGroupMessageMapper extends BaseMapperX<ImGroupMessageDO> {
     ...
     * @return 消息列表
     */
    default List<ImGroupMessageDO> selectExportList(ImGroupMessageManagerPageReqVO reqVO) {
        return selectList(new LambdaQueryWrapperX<ImGroupMessageDO>()
                .eqIfPresent(ImGroupMessageDO::getGroupId, reqVO.getGroupId())
                .eqIfPresent(ImGroupMessageDO::getSenderId, reqVO.getSenderId())
                .eqIfPresent(ImGroupMessageDO::getType, reqVO.getType())
    ...
    /**
     * 物理删除指定 groupId 群中由 senderId 发送的 messageIds 集合
     * <p>
     * 用于"群消息物理删除"场景：限制只在同群内、只删指定发送人，避免外部误传 groupId
     *
     * @param groupId    群编号
     ...
     * @return 实际删除条数
     */
    default int deleteByGroupIdAndSenderIdAndIds(Long groupId, Long senderId, List<Long> messageIds) {
        if (messageIds == null || messageIds.isEmpty()) {
            return 0;
        }
        return delete(new LambdaQueryWrapperX<ImGroupMessageDO>()
                .eq(ImGroupMessageDO::getGroupId, groupId)
                .eq(ImGroupMessageDO::getSenderId, senderId)
                .in(ImGroupMessageDO::getId, messageIds));
    }

    /**
     * 物理删除 groupId 群中（对调用者可见的）所有消息
     * <p>
     * 用于"清空群会话"场景；调用方在 Service 层按入群 / 退群时间做可见性过滤后再调用
     *
     * @param groupId 群编号
     * @return 实际删除条数
     */
