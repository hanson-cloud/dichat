...
public enum ImMessageTypeEnum implements ArrayValuable<Integer> {
    ...
    // ========== 社交支付消息（126-127 自有扩展段；OpenIM 未占用） ==========
    /**
     * 对应 OpenIM：无（自有扩展，社交支付消息）
     * 对应自己的类：无（payload 走 ImPrivateMessageDTO / ImGroupMessageDTO 顶层 content 字段）
     * 场景：发送红包后在聊天消息流中生成一条红包消息气泡；content 为 JSON({no, type, totalAmount, totalCount, blessing})
     * 气泡点击跳转红包详情页；领取状态由前端通过红包详情接口查询
     */
    RED_PACKET(126, "红包", true, true),
    /**
     * 对应 OpenIM：无（自有扩展，社交支付消息）
     * 对应自己的类：无（payload 走 ImPrivateMessageDTO 顶层 content 字段）
     * 场景：发起转账后在聊天消息流中生成一条转账消息气泡；content 为 JSON({no, amount, remark})
     * 气泡点击跳转转账详情页；收款状态由前端通过转账详情接口查询
     */
    TRANSFER(127, "转账", true, true),
    ...
    // ========== 参与方资料变更（2300 段位；方案 C 客服 / 机器人独立参与方资料同步） ==========
    /**
     * 对应 OpenIM：无（自有扩展）
     * 对应自己的类：无（payload 走 ImPrivateMessageDTO 顶层 senderType / senderId 字段）
     * 场景：人工客服 / 机器人资料（昵称 / 头像 / 在线状态等）变更后，广播给全部在线 C 端用户，
     * 由客户端据此重新拉取「联系客服」/ 机器人列表，保证会话内展示的客服 / 机器人资料实时一致。
     */
    PARTICIPANT_PROFILE_SYNC(2300, "参与方资料变更", false, false),

    // ========== 转人工会话切换信号（2301 段位；方案 C 机器人转人工 UX 增强） ==========
    /**
     * 对应 OpenIM：无（自有扩展）
     * 对应自己的类：无（payload 走 ImPrivateMessageDTO 顶层 content 字段，JSON {csUserId, csName}）
     * 场景：机器人转人工成功后，后端推一条信号给被转用户，前端据此自动切到人工客服会话（targetId = csUserId），
     * 否则用户仍停留在机器人会话窗口，看不到新建的客服会话。persistent=false、normal=false，不入库、不计未读。
     */
    TRANSFER_TO_HUMAN(2301, "转人工会话切换", false, false),
    ...
    /**
     * 对应 OpenIM：无直接对应（OpenIM 走 ConversationChangeNotification 1300 单聊路径）
     * 对应自己的类：PrivateMessageDeleteNotification
     * 场景：私聊消息被发送方物理删除；仅推当前用户多端，payload 携带 messageIds 让前端从本地缓存移除；不入库
     */
    PRIVATE_MESSAGE_DELETE(1540, "私聊消息删除", false, false),
    /**
     * 对应 OpenIM：无直接对应（OpenIM 走 ConversationChangeNotification 1300 单聊路径）
     * 对应自己的类：PrivateChatClearNotification
     * 场景：私聊会话被清空（currentUser 视角）；仅推当前用户多端，payload 携带 peerUserId 让前端从本地缓存移除该会话；不入库
     */
    PRIVATE_CHAT_CLEAR(1541, "私聊会话清空", false, false),
    /**
    ...
    /**
     * 对应 OpenIM：无直接对应（OpenIM 无单边删除 / 清空消息机制，自有扩展）
     * 对应自己的类：GroupMessageDeleteNotification
     * 场景：群消息被发送方物理删除；仅推当前用户多端，payload 携带 messageIds 让前端从本地缓存移除；不入库
     */
    GROUP_MESSAGE_DELETE(1542, "群消息删除", false, false),
    /**
     * 对应 OpenIM：无直接对应（OpenIM 无单边清空群消息机制，自有扩展）
     * 对应自己的类：GroupChatClearNotification
     * 场景：群会话被清空（currentUser 视角）；仅推当前用户多端，payload 携带 groupId 让前端从本地缓存移除该群消息；不入库
     */
    GROUP_CHAT_CLEAR(1543, "群会话清空", false, false);
    ...
    private static final Set<Integer> FRIEND_NOTIFICATION_TYPES = Set.of(
            ...
            FRIEND_UPDATE.type,
            // 1540/1541 段位虽属"会话管理"扩展，但 payload 继承 BaseFriendNotification，
            // 且通过 ofFriendNotification() 工厂方法发送，需纳入校验白名单
            PRIVATE_MESSAGE_DELETE.type,
            PRIVATE_CHAT_CLEAR.type);
     ...
     * 判断是否为需要审核的多媒体消息类型
     */
    public static boolean isMediaMessage(Integer type) {
        return type != null && (type.equals(IMAGE.type) || type.equals(VOICE.type)
                || type.equals(VIDEO.type) || type.equals(FILE.type));
    }
