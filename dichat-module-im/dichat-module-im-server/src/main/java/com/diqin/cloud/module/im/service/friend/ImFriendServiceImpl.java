...
import com.diqin.cloud.module.im.service.websocket.dto.notification.friend.*;
...
import static com.diqin.cloud.module.im.dal.redis.RedisKeyConstants.FRIEND_STATE;
...
public class ImFriendServiceImpl implements ImFriendService {
    ...
    @Override
    public void updateFriend(Long userId, AppImFriendUpdateReqVO reqVO) {
        // 1.1 校验：至少改一个字段（无字段变更，直接结束）
        if (reqVO.getDisplayName() == null && reqVO.getSilent() == null && reqVO.getPinned() == null
                && reqVO.getPhone() == null && reqVO.getRemark() == null) {
            return;
        }
        // 1.2 校验好友关系启用
        ...
        // 2. 更新好友属性（备注 / 免打扰 / 联系人置顶 / 备注电话 / 文字备注）
        friendMapper.updateById(new ImFriendDO().setId(friend.getId())
                .setSilent(reqVO.getSilent()).setDisplayName(reqVO.getDisplayName())
                .setPinned(reqVO.getPinned()).setPhone(reqVO.getPhone()).setRemark(reqVO.getRemark()));
    ...
    @Override
    public List<ImBlacklistDO> getBlockList(Long userId) {
        return blacklistMapper.selectListByUserId(userId);
    }

    @Override
    public void updateLastReadMessageId(Long userId, Long friendUserId, Long lastReadMessageId) {
        // 更新 (userId, friendUserId) 记录：表示 friendUserId 已读到 userId 发给他的 lastReadMessageId
        ImFriendDO friend = friendMapper.selectByUserIdAndFriendUserId(userId, friendUserId);
        if (friend != null) {
            friendMapper.updateById(new ImFriendDO().setId(friend.getId())
    ...
    @Override
    public Long getLastReadMessageId(Long userId, Long friendUserId) {
        ImFriendDO friend = friendMapper.selectByUserIdAndFriendUserId(userId, friendUserId);
        return friend != null ? friend.getLastReadMessageId() : null;
    }
